...
import third from './listToRange.third';
...
describe('listToRange', () => {
  ...
  test('third', () => {
    expect(third([1, 4, 5, 2, 3, 9, 8, 11, 0])).toBe('0-5,8-9,11');
    expect(third([1, 4, 3, 2])).toBe('1-4');
    expect(third([1, 4])).toBe('1,4');
    expect(third([1, 2])).toBe('1-2');
    expect(third([])).toBe('');
    expect(third([1])).toBe('1');
  });
});
